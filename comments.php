...
if ( ! empty( $_SERVER['SCRIPT_FILENAME'] ) && 'comments.php' == basename( $_SERVER['SCRIPT_FILENAME'] ) ) { /*1*/
		die( __( 'Please do not load this page directly. Thanks!', 'theme_textdomain' ) );
}
if ( post_password_required() ) { /*2*/ ?>
	<?php _e( 'This post is password protected. Enter the password to view comments.', 'theme_textdomain' ); ?>
	<?php
	return;
}
	...
	<div id="comments" class="comments-area">
		...
		if ( have_comments() ) :
			...
			if ( '1' === $theme_comment_count ) {
				printf(
					/* trans(intrs: 1: title. */
					__( 'One Responce', 'theme_textdomain' ),
					'<span>' . get_the_title() . '</span>'
				);
			} else {
				printf( // WPCS: XSS OK.
					/* trans(intrs: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s Responce', '%1$s Responces', $theme_comment_count, 'comments title', 'theme_textdomain' ) ),
